...
**Rules:**
- Use `getUserRoleFromDb(auth0Id)` — NEVER trust the `x-user-role` header for authorization decisions (see `.claude/docs/adr/0002-db-role-not-jwt-claim.md`).
...
- `.claude/docs/architecture/rbac.md` — how RBAC works overall
- `.claude/docs/adr/0002-db-role-not-jwt-claim.md` — why we read from DB
- `.claude/docs/reference/patterns/adding-a-new-role.md` — if your route introduces a new role entirely
